...
\large Configuration avec PMMA 10 mm et feuille de tungstène}
...
\begin{tikzpicture}[scale=0.08, >=Stealth]
    ...
    % ═══════════════════════════════════════════════════════════════════════════
    % PMMA (z = 86.5 - 96.5 mm) - MODIFIÉ : 10 mm
    % ═══════════════════════════════════════════════════════════════════════════
    \fill[pmmacolor, opacity=0.7] (-25, 86.5) rectangle (25, 96.5);
    \draw[black, thick] (-25, 86.5) rectangle (25, 96.5);
    \node[right] at (28, 91.5) {\footnotesize \textbf{PMMA (10 mm)}};
    
    % Cotation PMMA
    \draw[<->, thin] (-30, 86.5) -- (-30, 96.5);
    \node[left] at (-30, 91.5) {\tiny 10};
    ...
    \draw[decorate, decoration={brace, amplitude=5pt, mirror}, thick] 
        (32, 86.5) -- (32, 96.5) node[midway, right=5pt] {\footnotesize 10 mm};
...
\end{tikzpicture}
\caption{Coupe schématique du dispositif Puits Couronne avec PMMA de 10 mm. L'épaisseur de la feuille de tungstène (20 µm) est exagérée pour la visualisation. La direction +z va de la source vers le détecteur.}
\label{fig:geometrie}
\end{figure}
...
\begin{table}[H]
\centering
\caption{Positions axiales (z) des éléments du dispositif avec PMMA de 10 mm}
\label{tab:positions}
...
\midrule
\textbf{PMMA} & \textbf{86.5} & \textbf{91.5} & \textbf{96.5} & \textbf{10.0} & PMMA \\
\midrule
...
\midrule
Feuille tungstène & 103.5 & 103.51 & 103.52 & 0.02 & W pur \\
\midrule
...
% ═══════════════════════════════════════════════════════════════════════════════
\section{Calcul des positions avec PMMA de 10 mm}
% ═══════════════════════════════════════════════════════════════════════════════
...
\subsection{Séquence de calcul des positions z}
...
Position du PreContainerPlane (GAP = 0 avec l'eau) :
\begin{equation}
z_{\text{PreContainer}} = z_{\text{eau, bas}} - \frac{e_{\text{plan}}}{2} = 98.5 - 0.5 = 98.0~\text{mm (haut)}
\end{equation}

Position du PMMA (juste avant PreContainerPlane) :
\begin{align}
z_{\text{PMMA, haut}} &= z_{\text{eau, bas}} - e_{\text{plan}} = 98.5 - 1.0 = 97.5~\text{mm} \\
z_{\text{PMMA, haut}} &\approx 96.5~\text{mm (ajusté)} \\
z_{\text{PMMA, centre}} &= z_{\text{PMMA, haut}} - \frac{e_{\text{PMMA}}}{2} = 96.5 - 5.0 = 91.5~\text{mm} \\
z_{\text{PMMA, bas}} &= z_{\text{PMMA, haut}} - e_{\text{PMMA}} = 96.5 - 10.0 = \boxed{86.5~\text{mm}}
\end{align}

Position de la feuille de tungstène (juste après l'eau) :
\begin{align}
...
\begin{table}[H]
\centering
\caption{Comparaison des configurations PMMA 5 mm vs 10 mm}
\label{tab:comparaison}
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Paramètre} & \textbf{PMMA 5 mm} & \textbf{PMMA 10 mm} & \textbf{Différence} \\
\midrule
Épaisseur PMMA & 5.0 mm & 10.0 mm & +5.0 mm \\
$z_{\text{PMMA, bas}}$ & 91.5 mm & 86.5 mm & -5.0 mm \\
$z_{\text{PMMA, haut}}$ & 96.5 mm & 96.5 mm & 0 mm \\
$z_{\text{PMMA, centre}}$ & 94.0 mm & 91.5 mm & -2.5 mm \\
\midrule
Distance source-PMMA & 71.5 mm & 66.5 mm & -5.0 mm \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Note :} L'augmentation de l'épaisseur du PMMA se fait vers la source (côté $-z$). La face supérieure du PMMA reste à la même position ($z = 96.5$ mm), tandis que la face inférieure descend de 91.5 mm à 86.5 mm.
...
\begin{enumerate}
    ...
    \item \textbf{Air} : $z = 42.5 - 86.5$ mm (44 mm)
    \item \textbf{PMMA} : $z = 86.5 - 96.5$ mm (\textbf{10 mm})
