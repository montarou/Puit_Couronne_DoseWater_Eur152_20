...
\large Configuration avec PMMA 40 mm et feuille de tungstène}
...
\begin{tikzpicture}[scale=0.08, >=Stealth]
    ...
    % Cône d'émission (20°)
    \draw[sourcecolor, dashed, thick] (0, 20) -- (-12, 55);
    \draw[sourcecolor, dashed, thick] (0, 20) -- (12, 55);
    \node[sourcecolor, right] at (10, 38) {\footnotesize $\theta = 20°$};
    ...
    % ═══════════════════════════════════════════════════════════════════════════
    % PMMA (z = 56.5 - 96.5 mm) - MODIFIÉ : 40 mm
    % ═══════════════════════════════════════════════════════════════════════════
    \fill[pmmacolor, opacity=0.7] (-25, 56.5) rectangle (25, 96.5);
    \draw[black, thick] (-25, 56.5) rectangle (25, 96.5);
    \node[right] at (28, 76.5) {\footnotesize \textbf{PMMA (40 mm)}};
    
    % Cotation PMMA
    \draw[<->, thin] (-30, 56.5) -- (-30, 96.5);
    \node[left] at (-30, 76.5) {\tiny 40};
    ...
    \draw[decorate, decoration={brace, amplitude=5pt, mirror}, thick] 
        (32, 56.5) -- (32, 96.5) node[midway, right=5pt] {\footnotesize 40 mm};
    ...
    \draw[<->, dashed, gray] (0, 42.5) -- (0, 56.5);
    \node[gray, right] at (2, 49.5) {\footnotesize 14 mm (air)};

\end{tikzpicture}
\caption{Coupe schématique du dispositif Puits Couronne avec PMMA de 40 mm. L'épaisseur de la feuille de tungstène (20 µm) est exagérée pour la visualisation. La direction +z va de la source vers le détecteur.}
\label{fig:geometrie}
\end{figure}
...
\begin{table}[H]
\centering
\caption{Positions axiales (z) des éléments du dispositif avec PMMA de 40 mm}
\label{tab:positions}
...
\midrule
Air (gap) & 42.5 & -- & 56.5 & 14.0 & Air \\
\midrule
\textbf{PMMA} & \textbf{56.5} & \textbf{76.5} & \textbf{96.5} & \textbf{40.0} & PMMA \\
\midrule
...
\textbf{Épaisseur PMMA} & \textbf{40.0} & \textbf{mm} \\
...
% ═══════════════════════════════════════════════════════════════════════════════
\section{Calcul des positions avec PMMA de 40 mm}
% ═══════════════════════════════════════════════════════════════════════════════
...
Position du PMMA (extension vers la source avec épaisseur 40 mm) :
\begin{align}
z_{\text{PMMA, haut}} &= z_{\text{eau, bas}} - e_{\text{PreContainer}} = 98.5 - 1.0 - 1.0 = 96.5~\text{mm} \\
z_{\text{PMMA, centre}} &= z_{\text{PMMA, haut}} - \frac{e_{\text{PMMA}}}{2} = 96.5 - 20.0 = \boxed{76.5~\text{mm}} \\
z_{\text{PMMA, bas}} &= z_{\text{PMMA, haut}} - e_{\text{PMMA}} = 96.5 - 40.0 = \boxed{56.5~\text{mm}}
\end{align}
...
\begin{table}[H]
\centering
\caption{Comparaison des configurations PMMA 5 mm, 10 mm, 20 mm et 40 mm}
\label{tab:comparaison}
\begin{tabular}{@{}lccccc@{}}
\toprule
\textbf{Paramètre} & \textbf{5 mm} & \textbf{10 mm} & \textbf{20 mm} & \textbf{40 mm} & \textbf{$\Delta$ (5$\to$40)} \\
\midrule
Épaisseur PMMA & 5.0 & 10.0 & 20.0 & 40.0 mm & +35.0 mm \\
$z_{\text{PMMA, bas}}$ (mm) & 91.5 & 86.5 & 76.5 & \textbf{56.5} & -35.0 mm \\
$z_{\text{PMMA, haut}}$ (mm) & 96.5 & 96.5 & 96.5 & 96.5 & 0 mm \\
$z_{\text{PMMA, centre}}$ (mm) & 94.0 & 91.5 & 86.5 & \textbf{76.5} & -17.5 mm \\
\midrule
Distance source-PMMA (mm) & 71.5 & 66.5 & 56.5 & \textbf{36.5} & -35.0 mm \\
Gap air filtre-PMMA (mm) & 49.0 & 44.0 & 34.0 & \textbf{14.0} & -35.0 mm \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Note importante :} Avec un PMMA de 40 mm, le gap d'air entre le filtre et le PMMA n'est plus que de 14 mm. La face inférieure du PMMA ($z = 56.5$ mm) se rapproche significativement du filtre ($z_{\text{max}} = 42.5$ mm).
...
\begin{enumerate}
    ...
    \item \textbf{Air} : $z = 42.5 - 56.5$ mm (\textbf{14 mm} seulement)
    \item \textbf{PMMA} : $z = 56.5 - 96.5$ mm (\textbf{40 mm})
...
// APRÈS (nouvelle configuration 40 mm)
...
G4double pmmaCenterZ = pmmaTopZ - fPMMAThickness/2;          // 76.5 mm
G4double pmmaBottomZ = pmmaTopZ - fPMMAThickness;            // 56.5 mm
\end{verbatim}
...
\fbox{
\parbox{0.9\textwidth}{
\textbf{Attention :} Avec une épaisseur de PMMA de 40 mm, le gap d'air entre le filtre W/PETG et le PMMA n'est plus que de \textbf{14 mm}. 

Si l'épaisseur du PMMA dépasse \textbf{54 mm}, il y aura chevauchement avec le filtre ($z_{\text{PMMA, bas}} < z_{\text{filtre, haut}} = 42.5$ mm).

Vérifier que cette configuration est physiquement réalisable avant de lancer la simulation.
}
}
\end{center}
