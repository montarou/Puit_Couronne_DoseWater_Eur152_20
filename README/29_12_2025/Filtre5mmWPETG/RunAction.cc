...
static const G4double kNanoGrayFactor = 0.160218;
...
void RunAction::BeginOfRunAction(const G4Run* run)
{
    ...
    // Histogrammes de dose en nGy (indices 0-4)
    for (G4int i = 0; i < DetectorConstruction::kNbWaterRings; ++i) {
    ...
    // Histogramme dose totale (indice 5)
    analysisManager->CreateH1("h_dose_total", "Dose totale par désintégration [nGy]", 200, 0., 1.);
    
    // Histogrammes par raie gamma (indices 6-16)
    ...
    // ═══════════════════════════════════════════════════════════════
    // NTUPLE 0 : DOSES PAR DÉSINTÉGRATION
    // ═══════════════════════════════════════════════════════════════
    ...
    // ═══════════════════════════════════════════════════════════════
    // NTUPLE 1 : STATISTIQUES PAR RAIE GAMMA (fin de run)
    // ═══════════════════════════════════════════════════════════════
    ...
    // ═══════════════════════════════════════════════════════════════
    // NTUPLE 2 : PLAN PRE-CONTAINER (par désintégration)
    // Position : juste avant l'eau (GAP=0), matériau air
    // ═══════════════════════════════════════════════════════════════
    
    analysisManager->CreateNtuple("precontainer", "Plan PreContainer par désintégration");
    analysisManager->CreateNtupleIColumn("nPhotons");           // Photons vers eau (+z)
    analysisManager->CreateNtupleDColumn("sumEPhotons_keV");    // Somme énergie photons (keV)
    analysisManager->FinishNtuple();
    
    // ═══════════════════════════════════════════════════════════════
    // NTUPLE 3 : PLAN POST-CONTAINER (par désintégration)
    // Position : juste après l'eau (GAP=0), matériau W_PETG
    // ═══════════════════════════════════════════════════════════════
    
    analysisManager->CreateNtuple("postcontainer", "Plan PostContainer par désintégration");
    analysisManager->CreateNtupleIColumn("nPhotons_back");          // Photons depuis eau (-z)
    analysisManager->CreateNtupleDColumn("sumEPhotons_back_keV");   // Somme énergie photons depuis eau
    analysisManager->CreateNtupleIColumn("nElectrons_back");        // Électrons depuis eau (-z)
    analysisManager->CreateNtupleDColumn("sumEElectrons_back_keV"); // Somme énergie électrons depuis eau
    analysisManager->CreateNtupleIColumn("nElectrons_fwd");         // Électrons vers eau (+z)
    analysisManager->CreateNtupleDColumn("sumEElectrons_fwd_keV");  // Somme énergie électrons vers eau
    analysisManager->FinishNtuple();
    
    G4cout << "\n=== NTUPLES CRÉÉS ===" << G4endl;
    G4cout << "  0: doses          - Doses par désintégration" << G4endl;
    G4cout << "  1: gamma_lines    - Statistiques par raie gamma" << G4endl;
    G4cout << "  2: precontainer   - Plan PreContainer (avant eau, air)" << G4endl;
    G4cout << "  3: postcontainer  - Plan PostContainer (après eau, W_PETG)" << G4endl;
    G4cout << "=====================\n" << G4endl;
}
...
void RunAction::EndOfRunAction(const G4Run* run)
{
    ...
    // Remplir le ntuple des raies gamma (ntuple 1)
    for (G4int i = 0; i < EventAction::kNbGammaLines; ++i) {
                                      ...
                                      const std::array<G4double, 5>& ringEnergies)
{
    ...
    G4double totalMass_g = 0.;
    for (G4int i = 0; i < DetectorConstruction::kNbWaterRings; ++i) {
        totalMass_g += fRingMasses[i];
    }
    
    G4double eventDose_nGy = 0.;
    if (totalMass_g > 0. && totalDeposit > 0.) {
        eventDose_nGy = EnergyToNanoGray(totalDeposit / MeV, totalMass_g);
    }
    
    analysisManager->FillNtupleDColumn(0, DetectorConstruction::kNbWaterRings, eventDose_nGy);
    ...
    if (totalDeposit > 0.) {
        analysisManager->FillH1(5, eventDose_nGy);
    }
}

void RunAction::RecordPreContainerPlaneData(const PreContainerPlaneData& data)
{
    auto analysisManager = G4AnalysisManager::Instance();
    
    // Ntuple 2: precontainer
    analysisManager->FillNtupleIColumn(2, 0, data.nPhotons);
    analysisManager->FillNtupleDColumn(2, 1, data.sumEPhotons / keV);
    analysisManager->AddNtupleRow(2);
}

void RunAction::RecordPostContainerPlaneData(const PostContainerPlaneData& data)
{
    auto analysisManager = G4AnalysisManager::Instance();
    
    // Ntuple 3: postcontainer
    analysisManager->FillNtupleIColumn(3, 0, data.nPhotons_backward);
    analysisManager->FillNtupleDColumn(3, 1, data.sumEPhotons_backward / keV);
    analysisManager->FillNtupleIColumn(3, 2, data.nElectrons_backward);
    analysisManager->FillNtupleDColumn(3, 3, data.sumEElectrons_backward / keV);
    analysisManager->FillNtupleIColumn(3, 4, data.nElectrons_forward);
    analysisManager->FillNtupleDColumn(3, 5, data.sumEElectrons_forward / keV);
