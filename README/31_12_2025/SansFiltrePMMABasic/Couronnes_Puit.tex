...
\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.5]{Figures/histos_precontainer_FiltreWPETG5mm.png}
		\captionsetup{labelformat=empty}
...
\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.5]{Figures/histos_postcontainer_electrons_back_FiltreWPETG5mm.png}
		\captionsetup{labelformat=empty}
...
\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.5]{Figures/histos_postcontainer_electrons_fwd_FiltreWPETG5mm.png}
		\captionsetup{labelformat=empty}
...
\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.5]{Figures/histos_postcontainer_photons_FiltreWPETG5mm.png}
		\captionsetup{labelformat=empty}
...
\footnotesize PostContainerPlane&\footnotesize W/PETG&\footnotesize 103.52&\footnotesize 104.52&\footnotesize 1 mm &\footnotesize 25 \\
\rowcolor{wpetg!30}
...
\begin{tikzpicture}[>=Stealth, font=\small]
    ...
    % Labels anneaux
    \node[font=\small, blue!70!black] at ({2.5*\scaleR}, {(101+\zoff)*\scaleZ}) {\textbf{0}};
    \node[font=\small, blue!70!black] at ({7.5*\scaleR}, {(101+\zoff)*\scaleZ}) {\textbf{1}};
    \node[font=\small, blue!70!black] at ({12.5*\scaleR}, {(101+\zoff)*\scaleZ}) {\textbf{2}};
    \node[font=\small, blue!70!black] at ({17.5*\scaleR}, {(101+\zoff)*\scaleZ}) {\textbf{3}};
    \node[font=\small, blue!70!black] at ({22.5*\scaleR}, {(101+\zoff)*\scaleZ}) {\textbf{4}};
    ...
    % --- PostContainerPlane (1 mm, W/PETG) ---
    \fill[postplane, opacity=0.4] ({-\Rinner*\scaleR}, {(\zPostBot+\zoff)*\scaleZ + \Wthick}) rectangle ({\Rinner*\scaleR}, {(\zPostTop+\zoff)*\scaleZ + \Wthick});
    \draw[postplane!70!black, thick, dashed] ({-\Rinner*\scaleR}, {(\zPostBot+\zoff)*\scaleZ + \Wthick}) rectangle ({\Rinner*\scaleR}, {(\zPostTop+\zoff)*\scaleZ + \Wthick});
    ...
    \node[right, font=\small, water!50!black] at ({\cotexR}, {(101+\zoff)*\scaleZ}) {\textbf{5 mm}};
    
    % Tungstène : 20 µm (avec flèche vers zoom)
    \draw[->, thick, tungsten] ({\cotexR+0.8}, {(\zTungstenBot+\zoff)*\scaleZ + \Wthick/2}) -- ({\cotexR+2}, {(\zTungstenBot+\zoff)*\scaleZ + \Wthick/2 + 0.8});
    \node[right, font=\small, tungsten] at ({\cotexR+2}, {(\zTungstenBot+\zoff)*\scaleZ + \Wthick/2 + 0.8}) {\textbf{20 $\mu$m}};
    
    % PostContainerPlane : 1 mm
    \draw[<->, thick, postplane!70!black] ({\cotexR+0.8}, {(\zPostBot+\zoff)*\scaleZ + \Wthick}) -- ({\cotexR+0.8}, {(\zPostTop+\zoff)*\scaleZ + \Wthick});
    \node[right, font=\small, postplane!70!black] at ({\cotexR+0.8}, {(104+\zoff)*\scaleZ + \Wthick}) {\textbf{1 mm}};
    ...
    % Eau
    \node[font=\small\bfseries, water!60!black] at ({\labelx}, {(101+\zoff)*\scaleZ}) {\textbf{EAU}};
    
    % Tungstène
    \node[font=\footnotesize\bfseries, tungsten] at ({\labelx}, {(103.5+\zoff)*\scaleZ + \Wthick/2}) {\textbf{W}};
    
    % PostContainerPlane
    \node[font=\footnotesize, Carnelian!70!black] at ({\labelx+3.0}, {(104.0+\zoff)*\scaleZ + \Wthick}) {\textbf{PostContainer (W/PETG)}};
    ...
    \draw[->, very thick, red!70!black] (0, {(\zPMMAbot+\zoff-5)*\scaleZ}) -- (0, {(\zPMMAbot+\zoff-1)*\scaleZ});
    \node[below, font=\small, red!70!black] at (0, {(\zPMMAbot+\zoff-5)*\scaleZ}) {$\gamma$ (source)};
    
    % ═══════════════════════════════════════════════════════════════════════
    % TITRE
    % ═══════════════════════════════════════════════════════════════════════
    
    \node[above, font=\small\bfseries] at (0, {(\zContainerTop+\zoff)*\scaleZ}) {Coupe longitudinale du container (plan $r$-$z$)};
    ...
    \def\legspacing{0.45}
    \def\legstart{0.7}  % Espace après le titre "Légende"
    
    % PMMA (rectangle de y1 à y1-0.3, texte centré à y1-0.15)
    ...
    \node[right, font=\scriptsize] at ({\legx+0.6}, {\ypmma-0.15}) {PMMA (nouveau)};
    
    % Eau (rectangle de y2 à y2-0.3, texte centré à y2-0.15)
    ...
    % Tungstène (rectangle de y3 à y3-0.15, texte centré à y3-0.075)
    ...
    \node[right, font=\scriptsize] at ({\legx+0.6}, {\ytung-0.075}) {Tungstène 20 $\mu$m (nouveau)};
    
    % PreContainerPlane (rectangle de y4 à y4-0.25, texte centré à y4-0.125)
    ...
    % PostContainerPlane (rectangle de y5 à y5-0.25, texte centré à y5-0.125)
    \def\ypost{\legy-\legstart-1.8}
    \fill[postplane, opacity=0.4] ({\legx}, {\ypost}) rectangle ({\legx+0.5}, {\ypost-0.25});
    \draw[postplane!70!black, dashed] ({\legx}, {\ypost}) rectangle ({\legx+0.5}, {\ypost-0.25});
    \node[right, font=\scriptsize] at ({\legx+0.6}, {\ypost-0.125}) {PostContainerPlane (W/PETG)};
    
    % Container (rectangle de y6 à y6-0.25, texte centré à y6-0.125)
...
\noindent Le résultat $\bm{N_{\gamma,\text{back}}} = 0$ confirme \color{blue}\textbf{l'abscence de rétrodiffusion}\color{black} :
...
\midrule
\footnotesize Nombre moyen $\bm{\langle} \bm{N_{e^-}} \bm{\rangle}$&\footnotesize \num{3.9e-4}&\footnotesize \num{4.3e-4}&\footnotesize \textbf{0} \\
\footnotesize Écart-type $\bm{\sigma_{N_{e^-}}}$&\footnotesize \num{0.020}&\footnotesize \num{0.022}&\footnotesize 0 \\
\footnotesize Énergie moyenne $\bm{\langle} \bm{\Sigma} \bm{E_{e^-}} \bm{\rangle}$&\footnotesize \SI{360}{keV}&\footnotesize \SI{286}{keV} &\footnotesize \SI{0}{keV} \\
\footnotesize Entrées (énergie $> 0$) &\footnotesize $\sim 9750$&\footnotesize $\sim 10046$ &\footnotesize 0 \\
\bottomrule
\end{tabular}
\end{table}
...
\normalsize
\noindent \begin{mdframed}[backgroundcolor=orange!20]
\subsection{\color{blue}\textbf{Validation du Code}\color{black}}
\end{mdframed}
\footnotesize
%===============================================================================
\medskip
...
\begin{enumerate}
\item \color{blue}\textbf{Conservation approximative}\color{black} : Le rapport $\bm{N_\text{Post}}/\bm{N_\text{Pre}} \approx 0.87$ est cohérent avec les pertes géométriques latérales.
\item \color{blue}\textbf{Backscatter nul}\color{black} : $\bm{N_{\gamma,\text{back}}} = \bm{N_{e^-,\text{back}}} = 0$ confirme que :
\begin{itemize}
\item La détection de la direction ($\bm{p_z} > 0$ vs $\bm{p_z} < 0$) fonctionne
\item L'air ne produit pas de rétrodiffusion mesurable
\end{itemize}
\item \color{blue}\textbf{Distributions d'énergie}\color{black} : Les spectres PreContainer et PostContainer sont quasi-superposés, avec les raies caractéristiques de ${}^{152}$Eu visibles.
\end{enumerate}

\noindent Pour un cône d'émission de demi-angle $\bm{\theta_\text{max}} = 20°$ et une distance $\bm{\Delta z} = \SI{6}{mm}$, le rayon additionnel couvert est :

\begin{equation*}
\bm{\Delta r} = \bm{\Delta z} \cdot \tan(\bm{\theta_\text{max}}) = \bm{6} \times \tan(\bm{20}°) \approx \SI{2.2}{mm}
\end{equation*}

\noindent Les photons à la limite du cylindre ($\bm{r} \approx \SI{25}{mm}$) au PreContainer peuvent dépasser le rayon de détection au PostContainer, expliquant les pertes observées.
